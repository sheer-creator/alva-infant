...
/* Figma Modal/Connect 7908:69704 — 4 个 app,绑定 + 右上角下拉框单选接收方 */
const PLATFORMS: { id: AgentPlatform; name: string; sub: string; handle: string; logo: string }[] = [
...
function ReceiverSelect({ active }: { active: AgentPlatform | null }) {
  ...
  return (
    ...
    >
      <img src={meta.logo} alt="" className="size-[16px] shrink-0 rounded-full" />
...
function AppRow({
  ...
  return (
    <RowCard style={{ gap: 12 }}>
      <img src={platform.logo} alt="" className="size-[40px] shrink-0 rounded-full" />
      <div className="flex-1 min-w-0 flex flex-col gap-[4px]">
        ...
        <p className="text-[14px] leading-[22px] tracking-[0.14px] whitespace-nowrap overflow-hidden text-ellipsis" style={{ color: 'var(--text-n5, rgba(0,0,0,0.5))', fontFamily: SETTINGS_FONT }}>
          {connected ? platform.handle : platform.sub}
        </p>
      </div>
      <button
        type="button"
